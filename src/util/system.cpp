...
const char * const ILANCOIN_CONF_FILENAME = "ilancoin.conf";
const char * const ILANCOIN_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/ilancoin/ilancoin/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //ilancoin-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", ILANCOIN_SETTINGS_FILENAME);
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Blancoin
    // macOS: ~/Library/Application Support/Blancoin
    // Unix-like: ~/.ilancoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Blancoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Blancoin";
#else
    // Unix-like
    return pathRet / ".ilancoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", ILANCOIN_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Blancoin Core copyright is not removed by accident
    if (copyright_devs.find("Blancoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Blancoin Core developers";
    }
