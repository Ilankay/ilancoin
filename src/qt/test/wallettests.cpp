// Copyright (c) 2015-2019 The Ilancoin Core developers
...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount, bool rbf)
{
    ...
    entry->findChild<IlancoinAmountField*>("payAmount")->setValue(amount);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_ilancoin-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_ilancoin-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_ilancoin-qt  # macOS
void TestGUI(interfaces::Node& node)
{
...

    {
        ...
        QString balanceComparison = IlancoinUnits::formatWithUnit(unit, balance, false, IlancoinUnits::separatorAlways);
    ...
    // Amount input
    IlancoinAmountField* amountInput = receiveCoinsDialog.findChild<IlancoinAmountField*>("reqAmount");
    ...
    for (QWidget* widget : QApplication::topLevelWidgets()) {
        if (widget->inherits("ReceiveRequestDialog")) {
            ...
            QVERIFY(paymentTextList.at(1).indexOf(QString("URI: ilancoin:")) != -1);
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_ilancoin-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI(m_node);
