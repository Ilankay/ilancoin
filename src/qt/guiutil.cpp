...
#include <qt/ilancoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Blancoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new BlancoinAddressEntryValidator(parent));
    widget->setCheckValidator(new BlancoinAddressCheckValidator(parent));
}

bool parseBlancoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no ilancoin: URI
    if(!uri.isValid() || uri.scheme() != QString("ilancoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BlancoinUnits::parse(BlancoinUnits::ILC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseBlancoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseBlancoinURI(uriInstance, out);
}

QString formatBlancoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("ilancoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(BlancoinUnits::format(BlancoinUnits::ILC, info.amount, false, BlancoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openBlancoinConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", ILANCOIN_CONF_FILENAME));
    ...
    /* Open ilancoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Blancoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Blancoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Blancoin (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Blancoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "ilancoin.desktop";
    return GetAutostartDir() / strprintf("ilancoin-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a ilancoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Blancoin\n";
        else
            optionFile << strprintf("Name=Blancoin (%s)\n", chain);
