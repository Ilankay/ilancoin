...
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", ILANCOIN_PID_FILENAME)));
}
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", ILANCOIN_PID_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
...
/** Sanity checks
 *  Ensure that Ilancoin is running in a usable environment with all
...
{
    // Make sure only a single Ilancoin process is using the data directory.
...
bool AppInitMain(NodeContext& node)
{
    ...
    fs::path config_file_path = GetConfigFile(gArgs.GetArg("-conf", ILANCOIN_CONF_FILENAME));
