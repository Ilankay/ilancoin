...
static bool WalletAppInit(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        std::string strUsage = strprintf("%s ilancoin-wallet version", PACKAGE_NAME) + " " + FormatFullVersion() + "\n";
            if (!gArgs.IsArgSet("-version")) {
                strUsage += "\n"
                    "ilancoin-wallet is an offline tool for creating and interacting with " PACKAGE_NAME " wallet files.\n"
...
int main(int argc, char* argv[])
{
    ...
    if (method.empty()) {
        tfm::format(std::cerr, "No method provided. Run `ilancoin-wallet -help` for valid methods.\n");
        return EXIT_FAILURE;
    }
