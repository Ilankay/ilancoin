...
- [Development guidelines](#development-guidelines)
    - [General Ilancoin Core](#general-ilancoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Ilancoin Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play ilancoins" on the test network, if you
...
Ilancoin Core is a multi-threaded application, and deadlocks or other
...
Ilancoin Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as Ilancoin Core, where everyone uses
...
pay attention to for reviewers of Ilancoin Core code.

General Ilancoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: Ilancoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Some of these are maintained by active developers of Ilancoin Core, in which case changes should probably go
directly upstream without being PRed directly against the project. They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project. Changes to these should also
be sent upstream, but bugfixes may also be prudent to PR against Ilancoin Core so that they can be integrated
...
file descriptors, it will cause problems with Ilancoin's `select()` loop, because
...
compatibility between nodes. This happened in Ilancoin 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Ilancoin's
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Ilancoin Core, the wallet was always
