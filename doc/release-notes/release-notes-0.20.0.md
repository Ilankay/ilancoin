...
Blancoin Core version 0.20.0 is now available from:

  <https://ilancoincore.org/bin/ilancoin-core-0.20.0/>
...
using the Linux kernel, macOS 10.12+, and Windows 7 and newer.  Blancoin
Core should also work on most other Unix-like systems but is not as
frequently tested on them.  It is not recommended to use Blancoin Core on
unsupported systems.

From Blancoin Core 0.20.0 onwards, macOS versions earlier than 10.12 are no
longer supported. Additionally, Blancoin Core does not yet change appearance
...
- Instead of running `make` simply, you should instead run
  `ILANCOIN_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from Blancoin Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Blancoin Core version 0.18.0.
...
- Testing or debugging of implementations of the Blancoin P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Blancoin Core. Blancoin Core logs debug messages
...
- OpenSSL is no longer used by Blancoin Core.  (#17265)

- BIP70 support has been fully removed from Blancoin Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/ilancoin/ilancoin/issues/16599),
  [PR #16702](https://github.com/ilancoin/ilancoin/pull/16702), and the
  `ilancoind help` for more information.  This option is experimental and
...
- All custom settings configured when Blancoin Core starts are now
...
- The "Start Blancoin Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://ilancoincore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Blancoin Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Blancoin Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/ilancoin-core/HWI) for
  signing.  Future versions of Blancoin Core should support a GUI option
...
- Blancoin Core's automatically-generated source code documentation is
  now available at https://doxygen.ilancoincore.org.  (#17596)
...
- The `ilancoin-cli` utility used with the `-getinfo` parameter now
...
- The `ilancoin-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `ilancoinstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_ilancoin-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBILANCOIN_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_ilancoin` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBILANCOIN_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for ilancoin-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate ilancoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest ilancoin.conf (sdaftuar)
...
- #18022 Fix appveyor `test_ilancoin` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (ilancoin-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone ilancoin-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Blancoin Core files (hebasto)
...
- #17119 Fix broken ilancoin-cli examples (andrewtoth)
...
- #17503 Remove bitness from ilancoin-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.ilancoincore.org (laanwj)
...
- #17648 Rename wallet-tool references to ilancoin-wallet (hel-o)
...
- #18341 Replace remaining literal ILC with `CURRENCY_UNIT` (domob1812)
...
- #16115 On ilancoind startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from ilancoin-cli -getinfo (malevolent)
...
[Transifex](https://www.transifex.com/ilancoin/ilancoin/).
