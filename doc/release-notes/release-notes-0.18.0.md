Blancoin Core version 0.18.0 is now available from:
...
`/Applications/Blancoin-Qt` (on Mac) or `ilancoind`/`ilancoin-qt` (on
Linux).
...
Blancoin Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Blancoin Core on unsupported systems.

Blancoin Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, Blancoin Core does not yet change appearance when
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://ilancoincore.org/en/contact/) the Blancoin Core
...
`~ilancoin/.ilancoin`. This change makes Blancoin Core more consistent with
...
- A new document introduces Blancoin Core's BIP174 [Partially-Signed
  Blancoin Transactions
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Blancoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Blancoin Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Blancoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Blancoin Core, subsequently importing
...
- A new `ilancoin-wallet` tool is now distributed alongside Blancoin
...
This section describes planned changes to Blancoin Core that may affect
other Blancoin software and services.

- Since version 0.16.0, Blancoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Blancoin Core 0.20 (expected about
  a year after 0.18), Blancoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Blancoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Blancoin Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Blancoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Blancoin Core
...
- A sub-project of Blancoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Blancoin Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Blancoin Core's own implementation, although entropy
  gathered by Blancoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Blancoin Core a
...
- On macOS, Blancoin Core now opts out of application CPU throttling
...
- #11625 Add BlancoinApplication & RPCConsole tests (ryanofsky)
...
- #11526 Visual Studio build configuration for Blancoin Core (sipsorcery)
...
- #12775 Integration of property based testing into Blancoin Core (Christewart)
