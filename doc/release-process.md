...
* On both the master branch and the new release branch:
  ...
  - update `CLIENT_VERSION_MAJOR`, `PACKAGE_VERSION`, and `PACKAGE_STRING` in [`build_msvc/ilancoin_config.h`](/build_msvc/ilancoin_config.h)
...
* Update [`src/chainparams.cpp`](/src/chainparams.cpp) chainTxData with statistics about the transaction count and rate. Use the output of the `getchaintxstats` RPC, see
  [this pull request](https://github.com/ilancoin/ilancoin/pull/20263) for an example. Reviewers can verify the results by running `getchaintxstats <window_block_count> <window_final_block_hash>` with the `window_block_count` and `window_final_block_hash` from your output.
...
### Build and sign Blancoin Core for Linux, Windows, and macOS:
...
- Upload zips and installers, as well as `SHA256SUMS.asc` from last step, to the ilancoin.org server
...
- Update ilancoin.org version
  ...
  - First, check to see if the Blancoin.org maintainers have prepared a
  ...
  - If they have not prepared a release, follow the Blancoin.org release
...
- Announce the release:
  ...
  - Blancoin Core announcements list https://ilancoincore.org/en/list/announcements/join/

  - Update title of #ilancoin on Freenode IRC

  - Optionally twitter, reddit /r/Blancoin, ... but this will usually sort out itself
