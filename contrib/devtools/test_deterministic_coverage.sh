...
TEST_ILANCOIN_BINARY="src/test/test_ilancoin"
...
if [[ ! -e ${TEST_ILANCOIN_BINARY} ]]; then
    echo "Error: Executable ${TEST_ILANCOIN_BINARY} not found. Run \"./configure --enable-lcov\" and compile."
    exit 1
fi
...
while [[ ${TEST_RUN_ID} -lt ${N_TEST_RUNS} ]]; do
    ...
    if ! BOOST_TEST_RUN_FILTERS="${BOOST_TEST_RUN_FILTERS}" ${TEST_ILANCOIN_BINARY} > "${TEST_OUTPUT_TEMPFILE}" 2>&1; then
