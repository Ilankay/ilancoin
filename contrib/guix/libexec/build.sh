...
# Setup the directory where our Ilancoin Core build for HOST will occur
...
make -C depends --jobs="$MAX_JOBS" HOST="$HOST" \
                                   ...
                                   qt_config_opts_i686_linux='-platform linux-g++ -xplatform ilancoin-linux-g++'
...
if [ -z "$(find "${OUTDIR}/src" -name 'ilancoin-*.tar.gz')" ]; then
    ./autogen.sh
    env CONFIG_SITE="${BASEPREFIX}/${HOST}/share/config.site" ./configure --prefix=/
    make dist GZIP_ENV='-9n' ${V:+V=1}
    mkdir -p "${OUTDIR}/src"
    mv "$(find "${PWD}" -name 'ilancoin-*.tar.gz')" "${OUTDIR}/src/"
fi

# Determine the full path to our source tarball
SOURCEDIST="$(find "${OUTDIR}/src" -name 'ilancoin-*.tar.gz')"
# Determine our distribution name (e.g. ilancoin-0.18.0)
...
(
    ...
    # Setup the directory where our Ilancoin Core build for HOST will be
    # installed. This directory will also later serve as the input for our
    # binary tarballs.
    INSTALLPATH="${PWD}/installed/${DISTNAME}"
    mkdir -p "${INSTALLPATH}"
    # Install built Ilancoin Core to $INSTALLPATH
