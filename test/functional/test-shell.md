...
The `TestShell` submodule extends the `BlancoinTestFramework` functionality to
external interactive environments for prototyping and educational purposes. Just
like `BlancoinTestFramework`, the `TestShell` allows the user to:
...
to extend the object lifetime of the underlying `BlancoinTestFramework` between
...
## 2. Importing `TestShell` from the Blancoin Core repository

We can import the `TestShell` by adding the path of the Blancoin Core
...
The `TestShell` inherits all `BlancoinTestFramework` members and methods, such
as:
...
`BlancoinTestFramework` object. The full set of argument keywords which can be
...
Unlike the `BlancoinTestFramework` class, the `TestShell` keeps the underlying
Blancoind subprocesses (nodes) and logging utilities running until the user
explicitly shuts down the `TestShell` object.

During the time between the `setup` and `shutdown` calls, all `ilancoind` node
processes and `BlancoinTestFramework` convenience methods can be accessed
interactively.
...
underlying `BlancoinTestFramework`:
...
`BlancoinTestFramework` class. The user can override these in
...
| `rpc_timeout` | `60` | Sets the RPC server timeout for the underlying ilancoind processes. |
